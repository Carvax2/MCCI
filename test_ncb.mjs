async function test() {
  ...
  res = await fetch(`https://app.nocodebackend.com/api/data/update/sessions/${id}?Instance=${instance}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json', 'X-Database-Instance': instance, 'Authorization': `Bearer ${secret}` },
    body: JSON.stringify({ feedback: 'Updated Feedback' })
  });
  console.log("Update /update/sessions/id Response:", await res.text());

  res = await fetch(`https://app.nocodebackend.com/api/data/sessions/${id}?Instance=${instance}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json', 'X-Database-Instance': instance, 'Authorization': `Bearer ${secret}` },
    body: JSON.stringify({ feedback: 'Updated Feedback 2' })
  });
  console.log("Update /sessions/id Response:", await res.text());
}
test();
